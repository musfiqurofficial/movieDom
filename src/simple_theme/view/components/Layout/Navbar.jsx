...
import { useMediaQuery, If, Show } from "react-haiku";
import {
  DropMenu,
  Header,
  Logo,
...
const Navbar = () => {
  ...
  return (
    <>
      <Header className="py-2 position-sticky top-0">
        <Container
          fluid
          className="px-2 px-md-3 px-lg-5 d-flex gap-3 justify-content-between align-items-center"
        >
          <div>
            {smallDevice && (
            ...
            <NavLink to="/">
              <Logo
                src={LOGO_PATH}
                style={{ width: "100%", maxWidth: LOGO_WIDTH.value }}
              />
            </NavLink>
          </div>
              ...
              >
                <Offcanvas.Title>
                  <Image
                    src={LOGO_PATH}
                    alt=""
                    style={{ maxWidth: LOGO_WIDTH.value, width: "100%" }}
                  />
                </Offcanvas.Title>
              </Offcanvas.Header>
                  ...
                  {const_menu?.map((item) => (
                    <>
                      <Show>
                        ...
                        >
                          <NavDropdown title={item.name} color={theme.primary}>
                            {item?.drop_menu?.map((item) => (
                              <NavDropdown.Item href={item.url} target="_blink">
                                {item.name}
                              </NavDropdown.Item>
                            ))}
                          </NavDropdown>
