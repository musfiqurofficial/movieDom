...
export const getServerImgPath = (id, url, imgFor, type, higher) => {
  ...
  if (!imgFor) {
      ...
      if (imgFor.includes("movie")) {
          return `${MainServerURL}/Admin/main/images/${id}/${type.includes("poster") ? "poster" : "screen"
              }/${type.includes("poster") ? "" : ""}/${url}`;
      } else if (imgFor.includes("tv")) {
          return `${MainServerURL}/Admin/main/TVseries/${id}/${type.includes("poster") ? "poster" : "screen"
              }/${url}`;
      }
  }
};
